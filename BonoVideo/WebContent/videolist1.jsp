...
<div id="svw_main">
			...
			<c:choose>
   			<c:when test="${islive==1}">
   				<!--<s:property value="%{getText('video.livelist')}"/>-->视频列表
   			</c:when>
   			<c:otherwise>
   				<!--<s:property value="%{getText('video.vodlist')}"/>-->视频列表
   			</c:otherwise>
	    	</c:choose>
			</h2>
            <div class="post">
			...
			<div class="col one_fourth gallery_box" style="${video.videostate.cssstyle}">
                ...
                <!--<a href="javascript:if(confirm('Are you sure to Delete?'))location='VideoDelete.action?videoid=${video.id}'"><s:property value="%{getText('video.delete')}"/></a> -->
                </p>
			</div>
            </c:forEach>    
